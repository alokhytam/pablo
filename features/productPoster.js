...
const dataPath = path.join(__dirname, '../data/postingData.json');

let postingData = {};
if (fs.existsSync(dataPath)) {
  try {
    postingData = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));
  } catch (err) {
    console.error('❌  Gagal membaca postingData.json:', err);
  }
}

// Simpan data seller
postingData[posted.id] = {
...
try {
  fs.writeFileSync(dataPath, JSON.stringify(postingData, null, 2));
} catch (err) {
  console.error('❌  Gagal menyimpan postingData.json:', err);
}
